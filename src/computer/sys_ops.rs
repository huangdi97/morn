...
mod tests {
    ...
    #[test]
    fn test_set_wallpaper_simulated() {
        let result = set_wallpaper("/path/to/wallpaper.jpg");
        assert!(result.success);
        assert!(result.data.contains("/path/to/wallpaper.jpg"));
    }

    #[test]
    fn test_set_volume_clamps_to_100() {
        if !cfg!(target_os = "windows") {
            let result = set_volume(200);
            assert!(result.success);
    ...
    fn test_set_volume_normal() {
        if !cfg!(target_os = "windows") {
            let result = set_volume(75);
            assert!(result.success);
            assert!(result.data.contains("75%"));
        }
    }

    #[test]
    fn test_shutdown_simulated() {
        let result = shutdown(30);
        assert!(result.success);
        assert!(result.data.contains("30s"));
        assert!(result.approval_required);
    }

    #[test]
    fn test_sleep_simulated() {
        let result = sleep();
        assert!(result.success);
    ...
    #[test]
    fn test_get_volume_fallback() {
        let result = get_volume();
        assert!(result.success);
    }

    #[test]
    fn test_power_status_simulated() {
        let result = power_status();
        assert!(result.success);
    }

    #[test]
    fn test_network_status_returns_result() {
