...
impl NodeExecutor {
    pub fn execute(graph: &NodeGraph) -> Result<Vec<ExecutionResult>, String> {
        ...
        for entry in &entry_nodes {
            Self::topological_sort(entry.id.as_str(), graph, &node_map, &mut execution_order, &mut sort_visited)?;
        }
    ...
    pub(crate) fn execute_node(
        ...
        let output = match node.node_type {
            ...
            NodeType::LLMCall => {
                let model = node.config.get("model").and_then(|v| v.as_str()).unwrap_or("default");
                let prompt = node.config.get("prompt").and_then(|v| v.as_str()).unwrap_or("");
                serde_json::json!({
                    "model": model,
                    "prompt": prompt,
            ...
            NodeType::Loop => {
                let max_iterations = node.config.get("max_iterations").and_then(|v| v.as_u64()).unwrap_or(5);
            ...
            NodeType::Transform => {
                ...
                let input_value = inputs.get("data").cloned().unwrap_or(serde_json::json!(null));
                serde_json::json!({
            ...
            NodeType::Split => {
                let field = node.config.get("field").and_then(|v| v.as_str()).unwrap_or("items");
                let input_data = inputs.get("data").cloned().unwrap_or(serde_json::json!(null));
            ...
            NodeType::Code => {
                let code = node.config.get("code").and_then(|v| v.as_str()).unwrap_or("");
                let timeout = node.config.get("timeout").and_then(|v| v.as_u64()).unwrap_or(30);
                let sandbox = node.config.get("sandbox").and_then(|v| v.as_bool()).unwrap_or(true);
                serde_json::json!({
            ...
            NodeType::Trigger => {
                let trigger_type = if node.config.get("cron").is_some() { "cron" }
                    else if node.config.get("method").is_some() { "webhook" }
                    else { "manual" };
                serde_json::json!({
            ...
            NodeType::Wait => {
                let duration_secs = node.config.get("duration_secs").and_then(|v| v.as_u64()).unwrap_or(10);
            ...
            NodeType::Switch => {
                ...
                let input_value = inputs.get("value").and_then(|v| v.as_str()).unwrap_or("");
                let matched_case = cases.iter().position(|c| {
                    c.as_str().map(|s| s == input_value).unwrap_or(false)
                });
                let matched_case_value = matched_case.map(|i| serde_json::json!(i)).unwrap_or(serde_json::Value::Null);
                serde_json::json!({
                    "input": input_value,
                    "cases": cases,
