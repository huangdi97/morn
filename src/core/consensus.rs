...
impl ConsensusManager {
    ...
    pub fn publish(&self, session_id: &str, agent_id: &str, content: &str, stage: &str) -> Result<String, String> {
...

        {
            ...
            sessions.entry(session_id.to_string()).or_default().push(file);
        }

        Ok(file_name)
    }

    pub fn read(&self, session_id: &str, agent_id: &str, stage: &str) -> Result<Option<ConsensusFile>, String> {
        let file_name = format!("{}_{}_{}.consensus", session_id, agent_id, stage);
        let file_path = self.base_dir.join(&file_name);
    ...
    pub fn relay_to_next(&self, from_session: &str, from_agent: &str, to_session: &str, to_agent: &str, content: &str) -> Result<String, String> {
        self.publish(from_session, from_agent, content, "relay_out")?;
        self.publish(to_session, to_agent, content, "relay_in")?;
        Ok(format!("Relayed consensus from {}/{} to {}/{}", from_session, from_agent, to_session, to_agent))
    }
    ...
    pub fn derive_summary(&self, session_id: &str) -> Result<String, String> {
        ...
        match files {
            ...
            Some(files) => {
                ...
                stages.dedup();
                for stage in &stages {
                    let stage_files: Vec<&ConsensusFile> = files.iter().filter(|f| f.stage.as_str() == *stage).collect();
                    summary.push_str(&format!("Stage '{}' ({} contributors):\n", stage, stage_files.len()));
...
mod tests {
    ...
    fn test_publish_and_read() {
        ...
        mgr.publish("session-1", "agent-a", "data analysis complete", "analysis").unwrap();
    ...
    fn test_get_session_chain() {
        let mgr = create_test_manager();
        mgr.publish("session-1", "agent-a", "step1", "plan").unwrap();
        mgr.publish("session-1", "agent-b", "step2", "execute").unwrap();
        mgr.publish("session-2", "agent-c", "other", "test").unwrap();
    ...
    fn test_replay_session() {
        let mgr = create_test_manager();
        mgr.publish("session-1", "agent-a", "result", "done").unwrap();
    ...
    fn test_relay_to_next() {
        let mgr = create_test_manager();
        let relay = mgr.relay_to_next("session-1", "agent-a", "session-2", "agent-b", "handoff data").unwrap();
    ...
    fn test_find_consensus() {
        let mgr = create_test_manager();
        mgr.publish("s1", "a1", "revenue analysis shows growth", "report").unwrap();
        mgr.publish("s2", "a2", "code review complete", "qa").unwrap();
    ...
    fn test_derive_summary() {
        ...
        mgr.publish("s1", "a2", "execute step 2", "execute").unwrap();
