...
impl MornPlugin for HubPlugin {
    ...
    fn deps(&self) -> Vec<&str> {
        vec!["morn:data-layer", "morn:studio"]
    }
    ...
    fn init(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        let _storage: Storage = ctx.get::<Storage>("morn:storage").ok_or_else(|| {
            PluginError::LoadFailed("morn:hub".into(), "Storage not available".into())
        })?;
        Ok(())
    }
