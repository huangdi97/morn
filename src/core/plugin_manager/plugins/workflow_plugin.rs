...
impl MornPlugin for WorkflowPlugin {
    fn id(&self) -> &str { "morn:workflow" }
    fn deps(&self) -> Vec<&str> { vec!["morn:task-engine", "morn:registry"] }
    fn priority(&self) -> i32 { 135 }

    fn init(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        let registry = ctx.get::<Arc<Registry>>("morn:type-registry")
            .ok_or_else(|| PluginError::LoadFailed("morn:workflow".into(), "morn:type-registry not ready".into()))?;
    ...
    fn activate(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        ctx.get::<Arc<WorkflowBuilder>>("morn:workflow").ok_or_else(|| {
