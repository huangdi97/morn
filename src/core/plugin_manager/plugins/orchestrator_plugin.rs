...
impl MornPlugin for OrchestratorPlugin {
    fn id(&self) -> &str { "morn:orchestrator" }
    fn deps(&self) -> Vec<&str> { vec!["morn:task-engine", "morn:registry", "morn:memory"] }
    ...
    fn activate(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        ctx.get::<Arc<Mutex<Orchestrator>>>("morn:orchestrator").ok_or_else(|| {
