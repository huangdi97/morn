...
impl MornPlugin for SecurityPlugin {
    fn id(&self) -> &str { "morn:security" }
    fn deps(&self) -> Vec<&str> { vec!["morn:data-layer"] }
    fn priority(&self) -> i32 { 160 }
    ...
    fn activate(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        ctx.get::<Arc<SecurityGuard>>("morn:security-guard").ok_or_else(|| {
            PluginError::ActivateFailed("morn:security".into(), "morn:security-guard not registered".into())
        })?;
        Ok(())
    }
