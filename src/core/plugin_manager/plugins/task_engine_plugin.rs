...
impl MornPlugin for TaskEnginePlugin {
    fn id(&self) -> &str { "morn:task-engine" }
    fn deps(&self) -> Vec<&str> { vec!["morn:data-layer", "morn:sandbox"] }
    fn priority(&self) -> i32 { 145 }
    ...
    fn activate(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        ctx.get::<Arc<LongTaskEngine>>("morn:task-engine").ok_or_else(|| {
