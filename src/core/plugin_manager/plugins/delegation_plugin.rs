...
impl MornPlugin for DelegationPlugin {
    fn id(&self) -> &str { "morn:delegation" }
    fn deps(&self) -> Vec<&str> { vec!["morn:orchestrator"] }
    fn priority(&self) -> i32 { 105 }

    fn init(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        let registry = ctx.get::<Registry>("morn:type-registry")
            .ok_or_else(|| PluginError::LoadFailed("morn:delegation".into(), "morn:type-registry not ready".into()))?;
    ...
    fn activate(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        ctx.get::<Arc<DelegationManager>>("morn:delegation").ok_or_else(|| {
