...
impl MornPlugin for MemoryPlugin {
    fn id(&self) -> &str { "morn:memory" }
    fn deps(&self) -> Vec<&str> { vec![] }
    fn priority(&self) -> i32 { 165 }
    ...
    fn activate(&mut self, ctx: &PluginContext) -> Result<(), PluginError> {
        ctx.get::<Arc<Mutex<MemoryHub>>>("morn:memory-hub").ok_or_else(|| {
            PluginError::ActivateFailed("morn:memory".into(), "morn:memory-hub not registered".into())
        })?;
        Ok(())
    }
