...
pub enum WorkflowAction {
    LLMCall { system_prompt: String, model: String },
    ToolCall { tool_id: String, params: Value },
    AgentCall { agent_id: String, input: String },
    TeamCall { team_id: String, input: String },
    SubWorkflow { workflow_id: String },
    CodeExec { language: String, script: String },
    KnowledgeQuery { knowledge_id: String, query: String },
    HumanApproval { message: String },
    HumanInput { question: String },
    Notification { channel: String, message: String },
    Condition { expression: String, true_branch: Vec<WorkflowStep>, false_branch: Vec<WorkflowStep> },
    Loop { iterator: String, body: Vec<WorkflowStep>, max_iterations: u32 },
    Wait { duration_secs: u32 },
    Fork { branches: Vec<Vec<WorkflowStep>> },
    Join,
}
...
impl WorkflowTemplate {
    ...
    fn task_execution_template() -> WorkflowTemplate {
        WorkflowTemplate {
            ...
            description: "General task execution workflow with planning, execution, and review".into(),
            ...
            steps: vec![
                WorkflowStep {
                    id: "understand".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Analyze the user request and extract requirements.".into(), model: "default".into() },
                    depends_on: vec![],
                ...
                WorkflowStep {
                    id: "plan".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Create a step-by-step plan to fulfill the requirements.".into(), model: "default".into() },
                    depends_on: vec!["understand".into()],
                    timeout_secs: 15, retry_count: 1, approval_required: true,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "execute".into(),
                    action: WorkflowAction::AgentCall { agent_id: "default".into(), input: "".into() },
                    depends_on: vec!["plan".into()],
                    timeout_secs: 30, retry_count: 2, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "review".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Review the execution result and verify completeness.".into(), model: "default".into() },
                ...
                WorkflowStep {
                    id: "summarize".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Provide a concise summary of what was done.".into(), model: "default".into() },
                    depends_on: vec!["review".into()],
                    timeout_secs: 10, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "notify".into(),
                    action: WorkflowAction::Notification { channel: "default".into(), message: "Task completed".into() },
                    depends_on: vec!["summarize".into()],
                    timeout_secs: 5, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
            ],
        }
        ...
        WorkflowTemplate {
            ...
            steps: vec![
                ...
                WorkflowStep {
                    ...
                    action: WorkflowAction::LLMCall { system_prompt: "Filter and rank news items by relevance to the user's interests.".into(), model: "default".into() },
                ...
                WorkflowStep {
                    id: "summarize".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Summarize each important news item in 2-3 sentences.".into(), model: "default".into() },
                ...
                WorkflowStep {
                    ...
                    action: WorkflowAction::LLMCall { system_prompt: "Assess the potential impact of each news item.".into(), model: "default".into() },
                    depends_on: vec!["summarize".into()],
                    timeout_secs: 10, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "alert".into(),
                    action: WorkflowAction::Notification { channel: "default".into(), message: "".into() },
        ...
        WorkflowTemplate {
            ...
            steps: vec![
                WorkflowStep {
                    ...
                    action: WorkflowAction::LLMCall { system_prompt: "Analyze requirements and create specification.".into(), model: "default".into() },
                    depends_on: vec![],
                    timeout_secs: 20, retry_count: 1, approval_required: true,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "design".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Design architecture and component structure.".into(), model: "default".into() },
                ...
                WorkflowStep {
                    id: "implement".into(),
                    action: WorkflowAction::AgentCall { agent_id: "coder".into(), input: "".into() },
                    depends_on: vec!["design".into()],
                    timeout_secs: 60, retry_count: 2, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "test".into(),
                    action: WorkflowAction::CodeExec { language: "python".into(), script: "".into() },
                ...
                WorkflowStep {
                    id: "review".into(),
                    action: WorkflowAction::AgentCall { agent_id: "reviewer".into(), input: "".into() },
                ...
                WorkflowStep {
                    id: "deploy".into(),
                    action: WorkflowAction::Notification { channel: "default".into(), message: "Code ready for deployment".into() },
                    depends_on: vec!["review".into()],
                    timeout_secs: 5, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "document".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Generate documentation for the delivered code.".into(), model: "default".into() },
        ...
        WorkflowTemplate {
            ...
            steps: vec![
                WorkflowStep {
                    ...
                    action: WorkflowAction::ToolCall { tool_id: "web_search".into(), params: serde_json::json!({"query": ""}) },
                    depends_on: vec![], timeout_secs: 20, retry_count: 2, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "positioning".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Define product positioning and key messaging.".into(), model: "default".into() },
                    depends_on: vec!["market_research".into()], timeout_secs: 20, retry_count: 1, approval_required: true,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "launch_plan".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Create a comprehensive launch plan with timeline and milestones.".into(), model: "default".into() },
                    depends_on: vec!["positioning".into()], timeout_secs: 20, retry_count: 1, approval_required: true,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "materials".into(),
                    action: WorkflowAction::AgentCall { agent_id: "writer".into(), input: "".into() },
                    depends_on: vec!["launch_plan".into()], timeout_secs: 30, retry_count: 2, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "coordinate".into(),
                    action: WorkflowAction::Notification { channel: "default".into(), message: "Launch coordination".into() },
                    depends_on: vec!["materials".into()], timeout_secs: 10, retry_count: 2, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "launch".into(),
                    action: WorkflowAction::HumanApproval { message: "Ready to launch? Confirm to proceed.".into() },
                    depends_on: vec!["coordinate".into()], timeout_secs: 600, retry_count: 0, approval_required: true,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
            ],
        }
        ...
        WorkflowTemplate {
            ...
            steps: vec![
                WorkflowStep {
                    ...
                    action: WorkflowAction::LLMCall { system_prompt: "Analyze the proposal and extract key parameters.".into(), model: "default".into() },
                    depends_on: vec![], timeout_secs: 15, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "risk_assessment".into(),
                    action: WorkflowAction::AgentCall { agent_id: "analyst".into(), input: "".into() },
                    depends_on: vec!["proposal_analysis".into()], timeout_secs: 20, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "resource_eval".into(),
                    action: WorkflowAction::ToolCall { tool_id: "calc".into(), params: serde_json::json!({"expression": ""}) },
                    depends_on: vec!["proposal_analysis".into()], timeout_secs: 10, retry_count: 2, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "stakeholder_impact".into(),
                    action: WorkflowAction::AgentCall { agent_id: "researcher".into(), input: "".into() },
                    depends_on: vec!["risk_assessment".into()], timeout_secs: 20, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "recommendation".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Synthesize all evaluations into a go/no-go recommendation.".into(), model: "default".into() },
                    depends_on: vec!["resource_eval".into(), "stakeholder_impact".into()], timeout_secs: 15, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "present".into(),
                    action: WorkflowAction::Notification { channel: "default".into(), message: "Decision evaluation complete".into() },
                    depends_on: vec!["recommendation".into()], timeout_secs: 5, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
            ],
        }
        ...
        WorkflowTemplate {
            ...
            steps: vec![
                WorkflowStep {
                    ...
                    action: WorkflowAction::ToolCall { tool_id: "http_request".into(), params: serde_json::json!({"url": ""}) },
                    depends_on: vec![], timeout_secs: 15, retry_count: 2, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "analyze_metrics".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Analyze system metrics and identify anomalies.".into(), model: "default".into() },
                    depends_on: vec!["health_check".into()], timeout_secs: 15, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    id: "report".into(),
                    action: WorkflowAction::LLMCall { system_prompt: "Generate inspection report with status and recommendations.".into(), model: "default".into() },
                    depends_on: vec!["analyze_metrics".into()], timeout_secs: 10, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
                WorkflowStep {
                    ...
                    action: WorkflowAction::Condition {
                        ...
                        true_branch: vec![WorkflowStep {
                            id: "alert".into(),
                            action: WorkflowAction::Notification { channel: "default".into(), message: "Inspection found issues".into() },
                            depends_on: vec![], timeout_secs: 5, retry_count: 2, approval_required: false,
                            input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                        }],
                        false_branch: vec![],
                    },
                    depends_on: vec!["report".into()], timeout_secs: 5, retry_count: 1, approval_required: false,
                    input_mapping: HashMap::new(), output_mapping: HashMap::new(),
                },
            ],
        }
...
mod tests {
    ...
    fn test_workflow_action_serialization() {
        let action = WorkflowAction::ToolCall { tool_id: "web_search".into(), params: serde_json::json!({"q": "test"}) };
