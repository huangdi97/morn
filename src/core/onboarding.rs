...
impl Onboarding {
    ...
    pub fn select_model(
        ...
        if provider.trim().is_empty() || model.trim().is_empty() {
            return Err(MornError::Internal(
                "Provider and model cannot be empty".to_string(),
            ));
        }
    ...
    fn validate(&self) -> Result<(), MornError> {
        ...
        {
            return Err(MornError::Internal(
                "API key is required before completing onboarding".to_string(),
            ));
        }
        if self
            .draft
        ...
        {
            return Err(MornError::Internal(
                "Channel selection is required before completing onboarding".to_string(),
            ));
        }
        if self
            .draft
        ...
        {
            return Err(MornError::Internal(
                "Model selection is required before completing onboarding".to_string(),
            ));
        }
        Ok(())
    }
...
fn persist_onboarding_config(draft: &OnboardingDraft) -> Result<(), MornError> {
    ...
    fs::write(&path, &content)
        .map_err(|e| MornError::Internal(format!("Failed to write onboarding config: {}", e)))?;

    fs::metadata(&path)
        .map_err(|e| MornError::Internal(format!("Failed to verify config write: {}", e)))?;
