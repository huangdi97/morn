...
pub struct Marketplace {
    ...
    licenses: HashMap<String, Vec<License>>,
}
...
impl Marketplace {
    ...
    pub fn purchase(&mut self, listing_id: &str, user_id: &str) -> Result<License, String> {
    ...
    pub fn publish(&mut self, listing: Listing) -> Result<(), String> {
    ...
    pub fn install(&self, listing_id: &str, user_id: &str) -> Result<(), String> {
        let _listing = self.listings.get(listing_id).ok_or("Listing not found")?;
        let user_licenses = self.licenses.get(user_id).ok_or("No license found")?;
