...
impl Marketplace {
    ...
    fn list_builtin(&self) {
        ...
        for listing in builtin {
            if self.storage.get_listing(&listing.id).ok().flatten().is_none() {
    ...
    pub fn purchase(&self, listing_id: &str, user_id: &str) -> Result<License, String> {
        let listing = self.storage.get_listing(listing_id)?.ok_or("Listing not found")?;
    ...
    pub fn install(&self, listing_id: &str, user_id: &str) -> Result<(), String> {
        self.storage.get_listing(listing_id)?.ok_or("Listing not found")?;
    ...
    pub fn rate(&self, listing_id: &str, _user_id: &str, score: u8, _review: &str) -> Result<(), String> {
        let listing = self.storage.get_listing(listing_id)?.ok_or("Listing not found")?;
        let clamped_score = score.min(5) as f64;
        let new_rating = (listing.rating * listing.downloads as f64 + clamped_score) / (listing.downloads as f64 + 1.0);
        let new_downloads = listing.downloads + 1;
        self.storage.update_listing_rating(listing_id, new_rating, new_downloads)
    }

    pub fn search(&self, query: &str) -> Vec<Listing> {
        let q = query.to_lowercase();
        self.storage.list_listings(None).unwrap_or_default().into_iter().filter(|l| {
    ...
    pub fn install_to_registry(&self, listing_id: &str, registry: &mut Registry) -> Result<(), String> {
        let listing = self.storage.get_listing(listing_id)?.ok_or("Listing not found")?;
        let cap = Capability {
...
mod tests {
    ...
    fn test_install_to_registry() {
        ...
        let market = Marketplace::new(test_storage());
        market.purchase("listing-tool-web-search", "user-1").unwrap();
        market.install_to_registry("listing-tool-web-search", &mut registry).unwrap();
