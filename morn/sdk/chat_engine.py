...
class ChatEngine:
    def __init__(self, instance_name: str, memory_store, config: dict,
                 apz_store=None, quota_manager=None):
    ...
    async def _call_llm(self, messages: list) -> str:
        if self._quota_manager is not None:
            ...
            estimated = max(len(input_text) // 2, 10)
            if not self._quota_manager.check("S", estimated, "chat_engine"):
                self._logger.warning("quota exceeded — LLM call rejected")
                raise RuntimeError("QuotaExceeded: insufficient tokens")
            self._quota_manager.consume("S", estimated, "chat_engine")
