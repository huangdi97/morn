...
#[tauri::command]
pub(crate) fn delete_component(id: String, state: State<AppState>) -> Result<(), String> {
    let manager = state.manager.lock().map_err(|e| e.to_string())?;
    let mgr = manager
        .as_ref()
        .ok_or_else(|| "StudioManager not initialized".to_string())?;
...
#[tauri::command]
pub(crate) fn list_agent_templates(state: State<AppState>) -> Result<serde_json::Value, String> {
    let manager = state.manager.lock().map_err(|e| e.to_string())?;
    let mgr = manager
        .as_ref()
        .ok_or_else(|| "StudioManager not initialized".to_string())?;
...
#[tauri::command]
pub(crate) fn publish_component(id: String, state: State<AppState>) -> Result<(), String> {
