...
> 你的桌面 AI 创作系统
> 设计总纲 · 2026年6月 · v8.2 最终版（1.0 上线 + 插件体系）
...
三条原则：
- **系统即插件** — Morn 内置功能（DataLayer、Registry、Engine、Supervisor 等）也是插件，走同一套生命周期管理
- **双轨制** — 内核级用 Rust MornPlugin trait（全权限、编译安全），用户态用 Python/JS 脚本（低门槛、动态加载）
...
#### 第一轨：MornPlugin trait（Rust 内核模块）
...
生命周期的两个阶段：
1. **init** — 所有插件按任意顺序初始化，注册服务到 PluginContext
2. **activate** — 按拓扑排序的依赖顺序激活，确保依赖先就绪

**7 个内置内核插件**：

| 插件 ID | 职责 | 依赖 | 优先级 |
|---------|------|------|--------|
| `morn:data-layer` | Storage 数据库连接、first-run seed | 无 | 200 |
| `morn:registry` | TypeRegistry + Registry 注册中心 | data-layer | 190 |
| `morn:sandbox` | 代码沙箱执行 | data-layer | 185 |
| `morn:engine` | TaskEngine DAG 执行引擎 | data-layer, registry, sandbox | 160 |
| `morn:channel-bus` | 渠道适配器 + ConsoleBackend | data-layer | 180 |
| `morn:supervisor` | COO 主管 + 预设 Agent seed | data-layer, engine | 150 |
| `morn:studio` | Studio 创作台后端 | data-layer, registry, supervisor | 140 |

**插件通过 PluginContext 交换服务**（类型安全的 Any map）：

```rust
// 插件 A 注册服务
ctx.register("morn:storage", storage);

// 插件 B 获取服务
let storage = ctx.get::<Storage>("morn:storage");
```

#### 第二轨：BridgePlugin + PluginManager（外部脚本插件）

外部 Python/JS 插件通过 `plugins/` 目录 + `manifest.json` 加载：

```json
{
  "name": "my-plugin",
  "version": "0.1.0",
  "description": "A Morn plugin",
  "plugin_type": "tool",
  "entry": "main.py",
...
**BridgePlugin** 是一个特殊的 MornPlugin：
- `init()` 时扫描 `plugins/` 目录，发现所有子目录的 `manifest.json`
- `activate()` 时激活发现的外部插件
- 外部脚本通过 stdin/stdout JSON-RPC 与 Bridge 通信
- 权限由 manifest `permissions` 字段控制

**PluginManager** 是统一管理视图：

| 方法 | 行为 |
|------|------|
| `scan()` | 遍历 `plugins/` 读取各目录的 `manifest.json` |
| `load(name)` | 标记插件为已加载 |
| `activate(name)` | 标记为激活（theme 注入 CSS） |
| `deactivate(name)` | 停用插件 |
| `list()` | 列出全部插件（含 Rust 内核 + 外部脚本） |

### 11.3 插件类型

| 类型 | 注册到 | 说明 |
|------|--------|------|
| `theme` | CSS 变量覆盖 | 更换 UI 配色和视觉效果 |
| `channel` | ChannelRegistry | 新增 IM/社交/语音渠道 |
| `tool` | ToolRegistry | 新增 Agent 可用工具 |
| `knowledge` | 知识源注册表 | 新增外部知识源 |
| `ui_panel` | 前端插槽系统 | 新增界面面板 |
| `protocol` | 协议注册表 | 新增通信协议 |

### 11.4 加载流程（run() ~50 行）

```rust
pub fn run() {
    let mut plugins: Vec<Box<dyn MornPlugin>> = vec![
        Box::new(DataLayerPlugin), Box::new(RegistryPlugin),
        Box::new(SandboxPlugin), Box::new(EnginePlugin),
        Box::new(ChannelBusPlugin), Box::new(SupervisorPlugin),
        Box::new(StudioPlugin), Box::new(BridgePlugin::new(plugin_dir)),
    ];
    let ctx = PluginContext::new();
    load_plugins(&mut plugins, &ctx).expect("Plugin loading failed");

    // 所有插件注册到 PluginManager
    let mut pm = PluginManager::new(plugin_dir);
    pm.scan();
    for plugin in &plugins {
        pm.plugins.push(morn_plugin_to_plugin(plugin.as_ref()));
    }

    let state = AppState::from_ctx(&ctx);
    state.plugin_manager = Some(pm);
    // ... 启动 Tauri
}
```

**morn.exe 只做一件事：加载插件。** 所有功能都在插件里。
...
- 验证：`cargo build --lib` + `npm run build` + 1467 tests ✅
...
### 12.4 Plugin Architecture Batch（2026.06.17-06.19）

架构核心理念：**morn.exe = 只有 Plugin Loader（~50 行），其他全是插件。**

**Phases P0-P3（06.17-06.18）：插件基础设施**

| Phase | 内容 | 状态 |
|-------|------|------|
| P0（8 Batch） | MornPlugin trait + PluginContext + 拓扑排序 + 7 核心插件 | ✅ |
| P1（2 Batch） | catch_unwind 崩溃隔离 + enable/disable 运行时开关 + 前端管理面板 | ✅ |
| P2（2 Batch） | WASM 沙箱运行时（feature gate） | ✅ |
| P3（2 Batch） | Hub 集成：Plugin ListingType + install/uninstall 命令 + 存储层 | ✅ |

**Plugin Loader 精简（06.18-06.19）：**

| Batch | 内容 | 状态 |
|-------|------|------|
| B1 | 插件自治 — activate() 自己做 seed/接线 | ✅ |
| B2 | AppState::from_ctx() — 从 PluginContext 一键构造 | ✅ |
| B3 | 精简 run() — 删 106 行手动接线 | ✅ |
| B4 | 清理 import + clippy | ✅ |

run() 从 289 行缩至 183 行（Tauri 模板代码不可避），所有逻辑创建/seed/接线都在插件里。

**双轨插件系统统一（06.19）：**

| Batch | 内容 | 状态 |
|-------|------|------|
| B1 | PluginManifest 扩展 + plugins/ 目录 | ✅ |
| B2 | BridgePlugin — 外部 Python/JS 插件宿主 | ✅ |
| B3 | adapter.rs — MornPlugin → PluginManager 适配器 | ✅ |
| B4 | run() 统一加载流程（8 个插件含 Bridge） | ✅ |
| B5 | list_plugins 统一显示所有插件 | ✅ |

**最终验证：** cargo build --lib ✅ | 1467 tests ✅ | clippy 0 new warnings ✅

### 12.6 Phase 4.5 — 插件生态完善（2026.06.19-06.21）

架构理念落地后，补齐插件周边能力，让 31 个插件（16 系统 + 15 渠道）统一生命周期管理。

| Batch | 内容 | 状态 |
|-------|------|------|
| P4.5-B1 | **插件外部覆盖** — `register_external()` + `overrides.json`，允许第三方 JSON 配置替换任意插件 | ✅ |
| P4.5-B2 | **首次运行引导** — OnboardingWizard（4步向导）+ FeatureGuide（悬浮指引） | ✅ |
| P4.5-B3 | **备份插件** — `BackupPlugin` + SQLite 在线备份 API + `BackupPanel` 管理界面 | ✅ |
| P4.5-B4 | **Hub 插件** — `hub_plugin.rs` 将市场/发现功能包装为 MornPlugin | ✅ |
| P4.5-B5 | **15 个渠道插件** — Telegram/DingTalk/飞书/企业微信/QQBot 等统一通过 `channel_plugins.rs` 宏注册 | ✅ |
| P4.5-B6 | **统一生命周期系统** — `installed_items` 表（SQLite CRUD）+ `InstalledItemsPanel` 前端管理面板，系统插件/内容统一 install → enable/disable → uninstall | ✅ |
| P4.5-B7 | **清理设计文档** — 移除 `b*.md` Phase 设计文档，`tasks*.md` / `b[0-9]*.md` 加入 `.gitignore` | ✅ |

#### 验证
| 指标 | 值 |
|------|-----|
| 编译 | `cargo check -p morn` ✅ |
| Clippy | `cargo clippy -p morn` ✅ 0 warnings |
| 测试 | `cargo test --lib` ✅ 1467 passed |
| 前端 | `npm run build` ✅ |
| 插件总数 | 31（16 系统 + 15 渠道），统一 MornPlugin trait |
| CI | build-and-test ✅ build-tauri（Windows 安装包）⏳ 待跑通 |

### 12.5 代码健康 Batch（2026.06.16）
...
| 7 | **Hub 生态入口** | ✅ Hub 插件化（hub_plugin.rs），免费发布可用 |
| 20 | **生命周期管理** | ✅ installed_items + InstalledItemsPanel 统一管理所有插件/内容 |
...
安装 Morn → 运行 .exe
  → 检测到无 API Key
  → 显示欢迎页（状态 A）
  → 用户填写 API Key → 设置 key → Tauri setup()
